...
// Sanctioning bodies track pitch counts independently of each other — a kid's
// Little League all-star count doesn't carry into his BNS club team's, and vice
// versa. So workload lookups only ever pool games from the SAME rule set.
const ruleSet = (dv) => {
    const d = dv || "";
    if (/^LL /.test(d))
        return "LL";
...
function DugoutScorecard() {
    ...
    // No box score is attached, so it can't invent batting or pitching lines.
    const saveManualResult = (f) => {
        const aw = (f.awayName || "").trim() || "Visitors";
        const hm = (f.homeName || "").trim() || "Home";
        const pitchersFor = (side) => (f.pitchers || [])
        ...
        const rec = {
            ...
            gameTime: f.time || "",
            fieldName: (f.field || "").trim(),
            gameType: f.gameType || gameType || "season",
            eventName: (f.eventName || "").trim(),
            away: { name: aw, color: "", logo: "" },
            home: { name: hm, color: "", logo: "" },
            ...
            snapshot: {
                ...
                // mistake this for a scored game
                game: {
                    date: f.date || gameDate,
                    division: f.division || "",
                    gameType: f.gameType || gameType || "season",
                    eventName: (f.eventName || "").trim(),
                    pitchers: { away: pitchersFor("away"), home: pitchersFor("home") },
                },
            },
        };
        setGames((list) => {
    ...
    // The date is what season stats, the tournament board and the arms board
    // group by, so it has to update everywhere the record stores it.
    const setGameDateFor = (id, newDate) => {
        if (!newDate)
            return;
        setGames((list) => {
            const next = list.map((r) => {
                if (r.id !== id)
                    return r;
                const c = Object.assign({}, r, { date: newDate });
                if (c.snapshot && c.snapshot.game)
                    c.snapshot = Object.assign({}, c.snapshot, { game: Object.assign({}, c.snapshot.game, { date: newDate }) });
                return c;
            });
            persistGames(next);
            return next;
    ...
    const seasonTeamList = (typeFilter, eventFilter, yearFilter) => {
        const counts = {};
        games.forEach((rec) => {
            if (typeFilter && typeFilter !== "all" && recType(rec) !== typeFilter)
                return;
            if (eventFilter && eventFilter !== "all" && lc(recEvent(rec)) !== lc(eventFilter))
                return;
            if (yearFilter && yearFilter !== "all" && recYear(rec) !== yearFilter)
                return;
    ...
    const recDivision = (rec) => (rec && rec.snapshot && (rec.snapshot.division
        || (rec.snapshot.game && rec.snapshot.game.division))) || "";
    const pitcherHistory = (name, excludeId, rs) => {
        const key = lc(name);
        if (!key)
            return [];
        const out = [];
        games.forEach((rec) => {
            if (!rec || !rec.date || (excludeId && rec.id === excludeId))
                return;
            // only pool games governed by the same sanctioning body
            if (rs && ruleSet(recDivision(rec)) !== rs)
                return;
    ...
    const tourneyPitchers = (evName, dv) => {
        ...
        games.forEach((rec) => {
            if (!rec || lc(recEvent(rec)) !== key)
                return;
            // the game being scored is often ALSO in the archive (saved, then
            // reopened) — counting both double-counted every pitcher
            if (game && rec.id === game.id)
                return;
            // and never pool a different sanctioning body's games
            if (rs && ruleSet(recDivision(rec)) !== rs)
                return;
            const g2 = rec.snapshot && rec.snapshot.game;
            if (!g2 || !g2.pitchers)
    ...
    const yearList = () => {
        const seen = [];
        games.forEach((rec) => { const y = recYear(rec); if (y && seen.indexOf(y) === -1) seen.push(y); });
        return seen.sort((a, b) => b.localeCompare(a)); // newest first
    };
    // every distinct event name in the archive, for the season-stats picker
    const eventList = () => {
        const seen = [];
        games.forEach((rec) => { const e = recEvent(rec); if (e && !seen.some((x) => lc(x) === lc(e))) seen.push(e); });
        return seen.sort((a, b) => a.localeCompare(b));
    };
    // teamName === "*" aggregates every team matching the filters, tagging each
    // player with the club he played for so the table can be sorted by team.
    const computeSeason = (teamName, typeFilter, eventFilter, yearFilter) => {
        ...
        games.forEach((rec) => {
            const g = rec.snapshot && rec.snapshot.game;
            if (!g || rec.resultOnly)
                return; // a manually-entered final has no at-bats to count
            if (typeFilter && typeFilter !== "all" && recType(rec) !== typeFilter)
                return;
            if (eventFilter && eventFilter !== "all" && lc(recEvent(rec)) !== lc(eventFilter))
                return;
            if (yearFilter && yearFilter !== "all" && recYear(rec) !== yearFilter)
                return;
            ["away", "home"].forEach((side) => {
                if (!allTeams && lc(rec[side] && rec[side].name) !== tn)
                    return;
                gp += 1;
                const lu = (g.lineup && g.lineup[side]) || [];
                const st = (g.stats && g.stats[side]) || [];
                const sideTeam = ((rec[side] && rec[side].name) || "").trim();
                const addBat = (name, num, s, ext) => {
                    const k = allTeams ? lc(sideTeam) + "|" + lc(name) : lc(name);
                    if (!lc(name))
                        return;
                    ...
                    const b = bat[k] ||
                        (bat[k] = { name: (name || "").trim(), num: num || "", team: sideTeam, gp: 0, ab: 0, r: 0, h: 0, x2b: 0, x3b: 0, xhr: 0, rbi: 0, bb: 0, k: 0, hbp: 0, sac: 0 });
                    b.gp += 1;
                    b.ab += s.ab || 0;
                    b.r += s.r || 0;
                    b.h += s.h || 0;
                ...
                ((g.pitchers && g.pitchers[side]) || []).forEach((p) => {
                    const k = allTeams ? lc(sideTeam) + "|" + lc(p.name) : lc(p.name);
                    if (!lc(p.name))
                        return;
        ...
        React.createElement("div", { className: "shell" },
            ...
            licensed && phase === "setup" && (React.createElement(React.Fragment, null,
                ...
                React.createElement("div", { className: "setup-card limitcard" },
                    ...
                    (gameType === "tournament" || gameType === "playoff") && React.createElement("div", { className: "limitrow" },
                        React.createElement("input", { className: "dg-in", list: "dg-events", value: eventName, placeholder: "Name", onChange: (e) => setEventName(e.target.value), "aria-label": "Tournament or playoff name" }),
                        React.createElement("span", { className: "limithint" }, "name it to keep each event's stats separate"),
                        React.createElement("datalist", { id: "dg-events" }, eventList().map((n) => React.createElement("option", { key: n, value: n })))),
                    React.createElement("div", { className: "limitrow" },
            ...
            gamesOpen && (React.createElement("div", { className: "modal-back", onClick: () => setGamesOpen(false) },
                React.createElement("div", { className: "modal", onClick: (e) => e.stopPropagation() },
                    ...
                    React.createElement("button", { className: "dg ghost", style: { width: "100%", marginBottom: 10 }, onClick: () => setResultForm({
                            ...
                            ["home", "away"].forEach((sd) => {
                                (game.pitchers[sd] || []).forEach((p) => {
                                    if (!p.name || !p.name.trim())
                                        return;
                                    const hist = pitcherHistory(p.name, game.id, ruleSet(dv));
            ...
            seasonOpen &&
                (() => {
                    const data = computeSeason(seasonTeam, seasonType, seasonEvent, seasonYear);
                    ...
                    const copyText = () => {
                        ...
                        try {
                            navigator.clipboard.writeText(`${seasonTeam === "*" ? (seasonEvent !== "all" ? seasonEvent : "All teams") : seasonTeam} — Season stats\n${head}\n${body}`);
                        }
                        catch (_a) { }
                    };
                    return (React.createElement("div", { className: "modal-back", onClick: () => setSeasonOpen(false) },
                        React.createElement("div", { className: "modal season-modal", onClick: (e) => e.stopPropagation() },
                            ...
                            React.createElement("div", { className: "season-controls" },
                                React.createElement("select", { className: "dg-in", value: seasonTeam, onChange: (e) => setSeasonTeam(e.target.value), "aria-label": "Team" },
                                    React.createElement("option", { value: "*" }, seasonEvent !== "all" ? "All teams in event" : "All teams"),
                                    seasonTeamList(seasonType, seasonEvent, seasonYear).map((t) => (React.createElement("option", { key: t, value: t }, t)))),
                            ...
                            React.createElement("div", { className: "season-tabs", style: { marginTop: 6 } }, [["all", "All"], ["season", "Season"], ["playoff", "Playoff"], ["tournament", "Tourney"]].map(([v, lbl]) => React.createElement("button", { key: v, className: `dg ${seasonType === v ? "" : "ghost"}`, style: { fontSize: 12, padding: "6px 4px" }, onClick: () => { setSeasonType(v); if (v === "season" || v === "all") setSeasonEvent("all"); setSeasonTeam("*"); } }, lbl))),
                            React.createElement("div", { style: { display: "grid", gridTemplateColumns: eventList().length > 0 ? "1fr 1fr" : "1fr", gap: 6, marginTop: 6 } },
                                React.createElement("select", { className: "dg-sel", value: seasonYear, onChange: (e) => setSeasonYear(e.target.value), "aria-label": "Filter by year" },
                                    React.createElement("option", { value: "all" }, "All years"),
                                    yearList().map((y) => React.createElement("option", { key: y, value: y }, y))),
                                eventList().length > 0 && React.createElement("select", { className: "dg-sel", value: seasonEvent, onChange: (e) => { setSeasonEvent(e.target.value); setSeasonTeam("*"); }, "aria-label": "Filter by event" },
            ...
            resultForm && (() => {
                ...
                const setP = (i, k, v) => { const ps = f.pitchers.slice(); ps[i] = Object.assign({}, ps[i], { [k]: v }); set("pitchers", ps); };
                ...
                return (React.createElement("div", { className: "modal-back", onClick: () => setResultForm(null) },
                    React.createElement("div", { className: "modal set-modal", onClick: (e) => e.stopPropagation() },
                        ...
                        React.createElement("div", { className: "limitrow" },
                            ...
                            React.createElement("input", { className: "dg-in", type: "number", inputMode: "numeric", placeholder: "R", value: f.awayRuns, onChange: (e) => set("awayRuns", e.target.value), "aria-label": "Visiting runs" })),
                        React.createElement("div", { className: "limitrow" },
                            React.createElement("input", { className: "dg-in", placeholder: "Home team", value: f.homeName, onChange: (e) => set("homeName", e.target.value), "aria-label": "Home team" }),
                            React.createElement("input", { className: "dg-in", type: "number", inputMode: "numeric", placeholder: "R", value: f.homeRuns, onChange: (e) => set("homeRuns", e.target.value), "aria-label": "Home runs" })),
                        React.createElement("div", { className: "limitrow" },
                            React.createElement("input", { className: "dg-in", type: "date", value: f.date, onChange: (e) => set("date", e.target.value), "aria-label": "Date" }),
                            React.createElement("input", { className: "dg-in", type: "time", value: f.time, onChange: (e) => set("time", e.target.value), "aria-label": "Time" })),
                        React.createElement("div", { className: "limitrow" },
                        ...
                        f.pitchers.map((p, i) => React.createElement("div", { className: "limitrow", key: i, style: { gridTemplateColumns: "78px 1fr 62px" } },
                            React.createElement("select", { className: "dg-sel", value: p.side, onChange: (e) => setP(i, "side", e.target.value), "aria-label": "Team" },
                                React.createElement("option", { value: "away" }, teamOf("away").slice(0, 10)),
                                React.createElement("option", { value: "home" }, teamOf("home").slice(0, 10))),
                            React.createElement("input", { className: "dg-in", placeholder: "Pitcher", value: p.name, onChange: (e) => setP(i, "name", e.target.value), "aria-label": "Pitcher name" }),
